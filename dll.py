...
def display():
    ...
    plt.plot(vi.x,vi.y,marker='x',c='r',ms=12)
    P.plot()
    K.kplot()
    plt.show()
...
if(len(sys.argv) == 3):
    N = int(sys.argv[2])
    plt.text(6,21,"Please provide input.",fontsize=font_size)
    plt.axis([0,win_size,0,win_size])
    plt.plot()
...
#inpt = [(3.3266129032258061, 4.312770562770563), (5.8870967741935489, 9.3722943722943732), (8.5887096774193559, 5.7196969696969706), (11.25, 2.2294372294372291), (11.754032258064516, 8.425324675324676), (15.7258064516129, 8.9935064935064943), (15.06048387096774, 13.971861471861473), (18.004032258064516, 17.516233766233768), (10.806451612903224, 19.410173160173162), (3.689516129032258, 18.463203463203467), (6.8145161290322571, 16.677489177489178), (8.870967741935484, 15.703463203463205), (9.4153225806451601, 12.943722943722944), (3.991935483870968, 13.430735930735931)]
#inpt = [(1.774193548387097, 3.9339826839826841), (6.1088709677419351, 5.7196969696969706), (10.362903225806452, 4.0422077922077921), (14.939516129032256, 2.283549783549784), (12.439516129032256, 11.563852813852815), (16.25, 15.405844155844157), (5.0604838709677411, 18.814935064935067), (8.0241935483870961, 13.620129870129873)]
x = [i[0] for i in inpt]
y = [i[1] for i in inpt]
...
if(r_node is None):
    plt.text(2,10,"A convex polygon coincides with its kernel.",fontsize=font_size,bbox=dict(facecolor='red', alpha=0.5))
    P.plot()
    plt.axis([0,win_size,0,win_size])
    plt.show()
    sys.exit()
    ...
    if(is_reflex(vi)): #vertex i is reflex
        ...
        if(left_test<0 or math.isclose(left_test,0)): #F1 lies on or right of the vi+1 to inf line
            ...
            while(wt1!=L1):
                ...
                wt2 = wt2.left
            if(wt1==L1):
                plt.text(text_x,text_y,text,fontsize=font_size,bbox=dict(facecolor='red', alpha=0.5))
                display()
                sys.exit(0)
            ws2 = F1
            ws1 = F1.right
